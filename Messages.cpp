...
std::vector<ObstacleState> GetObstacles(const std::string & str)
{
	...
	for(int i = 0; i < count; i++) {
		...
		if(sscanf(param.c_str(), "[%f,%f,%f]", &obj.x, &obj.z, &obj.radius) == 3)
			ret.push_back(obj);
		else
...
std::deque<GoalState> GetGoals(const std::string & str)
{
	std::deque<GoalState> ret;
	std::stringstream ss(str);
	int count = 0;
	ss >> count;
	for (int i = 0; i < count; i++) {
		...
		if (sscanf(param.c_str(), "[%f,%f]", &obj.x, &obj.z) == 2)
			ret.push_back(obj);
		else
			std::cerr << "Error in transmission of goal state." << std::endl;
	}
	return ret;
}
